...
const app=initializeApp(firebaseConfig),db=getDatabase(app),$=s=>document.querySelector(s),$$=s=>[...document.querySelectorAll(s)];let state={sessions:{},meta:{}},dirty=false;
const esc=v=>String(v??'').replace(/[&<>'"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c]));
function toast(t){const e=$('#toast');e.textContent=t;e.classList.add('show');setTimeout(()=>e.classList.remove('show'),2600)}
function blank(){const s={};for(let i=1;i<=8;i++)s[i]={title:`Class ${i}`,date:'',status:'not-started',notes:'',resources:[],attendance:{},review:{}};return{sessions:s,meta:{}}}
function normalize(){state.sessions??={};state.meta??={};for(let i=1;i<=8;i++){state.sessions[i]??={};const s=state.sessions[i];s.title??=`Class ${i}`;s.date??='';s.status??='not-started';s.notes??='';s.resources=Array.isArray(s.resources)?s.resources:[];s.attendance??={};s.review??={}}}
function markDirty(){dirty=true;$('#dirtyLabel').textContent='Unsaved changes — click Save to publish.';$('#dirtyLabel').classList.add('warning')}
function renderMatrix(){normalize();$('#matrix thead').innerHTML=`<tr><th>Student</th>${Array.from({length:8},(_,x)=>`<th>${esc(state.sessions[x+1].title||`Class ${x+1}`)}<br><small>A / R</small></th>`).join('')}</tr>`;$('#matrix tbody').innerHTML=students.map(st=>`<tr><td>${st}</td>${Array.from({length:8},(_,x)=>{const i=x+1,s=state.sessions[i];return `<td><div class="checkcell"><label title="Attended">A <input class="check" type="checkbox" data-s="${st}" data-i="${i}" data-k="attendance" ${s.attendance[st]?'checked':''}></label><label title="Reviewed">R <input class="check" type="checkbox" data-s="${st}" data-i="${i}" data-k="review" ${s.review[st]?'checked':''}></label></div></td>`}).join('')}</tr>`).join('');$$('.check').forEach(x=>x.onchange=markDirty)}
...
$('#importFile').onchange=async e=>{const f=e.target.files[0];if(!f)return;try{state=JSON.parse(await f.text());normalize();render();markDirty();toast('Backup loaded. Click Save to publish.')}catch{toast('Invalid backup file')}};
